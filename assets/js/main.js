...
(function (typedoc) {
    ...
    const urlVersionMatch = document.location.pathname.match(/v[0-9]+\.[0-9]+/);
    const urlVersion = urlVersionMatch ? urlVersionMatch[0] : ""
    if (typeof window.localStorage != 'undefined') {
        window.localStorage.setItem('version', urlVersion);
    }
    var Filter = (function (_super) {
        ...
        function Filter(options) {
            ...
            _this.optionVersion = new FilterItemSelect('version', "Version");
            _this.optionVersion.handleValueChange = function (oldValue, newValue) {
                // console.log("change", oldValue, newValue)
                try {
                    this.__proto__.handleValueChange.call(this, oldValue, newValue)
                } catch (err) {
                    console.log(err);
                }
                const pathname = document.location.pathname.replace(/^\/+(v[0-9]+\.[0-9]+\/)?/, `/${newValue ? newValue + "/" : ""}`);
                // console.log({oldValue, newValue, pathname})
                Promise.resolve().then(() => document.location.pathname = pathname);
            }
    ...
    var Signature = (function (_super) {
        ...
        function Signature(options) {
            ...
            if (_this.container) {
                ...
                _this.el.querySelectorAll('.tsd-signature').forEach(function (signature) {
        ...
        Signature.prototype.createGroups = function () {
            var _this = this;
            var signatures = this.el.querySelectorAll('.tsd-signature');
