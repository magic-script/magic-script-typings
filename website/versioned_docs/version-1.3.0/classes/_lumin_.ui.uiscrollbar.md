---
id: "_lumin_.ui.uiscrollbar"
title: "UiScrollBar"
sidebar_label: "UiScrollBar"
---
...
The indication of where the thumb is along its track.
...
The indication for the size of the thumb.
...
Name | Type | Description |
------ | ------ | ------ |
`value` | number | a normalized value between 0 and 1, according to which the thumb will be placed along the track.  |
...
Name | Type | Description |
------ | ------ | ------ |
`value` | number | The ratio of scroll bound width/height over the scroll content width/height.  |
...
▸ **Create**(`prism`: [Prism](_lumin_.prism.md), `width`: number, `height?`: number): *[UiScrollBar](_lumin_.ui.uiscrollbar.md)*
...
`width` | number | The length of the scrollbar. This is a little bit longer than the scroll range (which is the range the thumb will run) |
`height?` | number | `default = 0.0`<br/> - This is how wide the scrollbar will be. A user should normally not care about this unless they intend to change the skin of an app |

**Returns:** *[UiScrollBar](_lumin_.ui.uiscrollbar.md)*

The new UiScrollBar node.
